...
# My Idea: 
Refer to "Environment.md" for the development option to create S3FileDownloader.py to cover the following tasks:
Download all files from a specific S3 prefix, expect 4 arguments: 
1) S3 Prefix 
2) Local Folder to store the downloaded objects
3) Size Filter, format [Min, Max], only download S3 object size > Min (MB), but <=Max (MB)
4) Download Concurrentcy M, define the max concurrency (e.g., 4) to do the parallel downloading from S3 

Save relevant AWS credential in a secret file which should not be checked into Github
