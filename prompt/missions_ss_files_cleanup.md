...
# My Idea: 
Refer to "Environment.md" for the development option to create ss-files-cleanup.py to cover the following tasks:
在指定目录中clean up file names
1. Renames all files whose prefix is listed in data\ss-files-cleanup-prefixes2Remove.csv to new file name by removing the prefix segment. If hit duplicated name, attach _copy{n} to file name tail 
2. Group similar files if they contains max # of matched segments, put into subfolder {nnn}, nnn increase from 001 to 999 
