...
# My Idea: 
Refer to "Environment.md" for the development option to create gdrive-local-sync.py to 
1. Sync all documents in Google Drive (URL is the 1st argument) to local folder (full path is the 2nd argument), using the Google credential. If the document already exist and same size, skip sync; otherwise overwrite the local copy
2. Since personal Google credentail is used, find a way to well protect the credential. 
