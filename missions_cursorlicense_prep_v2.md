...
# My Idea: 
Refer to "Environment.md" for the development option to create license-prep-v2.py to cover the following tasks:
1. 从 data\input.xlsx 读取
2. From tab "Cursor_Credits_Justification", extract emails from column "users_to_add" if "Note" column is not null, output as justified.csv
3. 匹配 tab "Token Usage" 的 "Email" column and tab "Cursor Licenses" 的 "Users_to_add" column（不区分大小写），追加 Total Prompts 列, Fast Premium Prompts列, 和 On-Demand Spend 列
4. 在扩展后的 Cursor Licenses 上 add a new colum "Total" = "Total Prompts"+ "Fast Premium Prompts", then 升序排序 by "Total" column
5. 先将所有 monthly-spend-limit 视为 0，再逐行按档位赋值:
First loop: 
If "Users_to_add" column is not in justified.csv, continue
Else If "On-Demand Spend" > 100, set "monthly-spend-limit" as 150. 
Else If "On-Demand Spend" > 50, set "monthly-spend-limit" as 100. 
Else if "On-Demand Spend" > 20, set "monthly-spend-limit" as 50
Else if "On-Demand Spend" > 10, set "monthly-spend-limit" as 30
Else if "On-Demand Spend" > 2 , set "monthly-spend-limit" as 15
Else, continue
Sencond loop: 
If "Users_to_add" column is not in justified.csv, continue
Else if "Total" < 400, continue
Else if "Total" < 600, increase "monthly-spend-limit" by 10
Else if "Total" < 1000, increase "monthly-spend-limit" by 40
Else if "Total" < 1500, increase "monthly-spend-limit" by 80
Else if "Total" < 2000, increase "monthly-spend-limit" by 120
